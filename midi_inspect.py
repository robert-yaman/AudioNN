...
def patternWithOneNoteTrackFromPattern(pattern):
  # Takes a MIDI track with multiple note tracks (e.g. for a Bach fugure) and combines them. Returns a pattern with the unified track
  ...
  for track in note_tracks:
    track_abs_times[track[0].text] = 0 #confirm these are always unique 
  total_time = 0
